            ...
            switch (_a.label) {
                case 0:
                    ...
                    fs_extra_1["default"].ensureDir(dirs.targetDirectory).then(function () {
                        console.log("%s target folder of %s.", chalk_1["default"].cyan('CHECK'), dirs.targetDirectory);
                    })["catch"](function (err) {
                        console.error('%s Base Folder %s not in current directory and could not be created. %s', chalk_1["default"].red.bold('ENSURE-DIR'), dirs.targetDirectory, err);
                    });
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 3, , 4]);
